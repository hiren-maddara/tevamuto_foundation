...
const getArticleData = async (section, id) => {
    if (section === 'about') {
        ...
        if(error){
            // loading.value = false
            return loadingError.value = true
        }
    }
...
articleData.value = await getArticleData(sect.value, articleId.value)
// console.log(articleData.value.content)

//then get other articles
const loadArticles = async (sect) => {
    if (sect === 'about') {
        const { error, data } = await supabase.from('about').select().order('created_at', {
            ascending: false
        })
...
const recentArticles = ref(await loadArticles(sect.value))


watch(() => route, async () => {
    articleId.value = route.params.id
    articleData.value = ref(await getArticleData(sect.value, articleId.value))
})
</script>

<template>
    <section v-if="loading" class="px-5 py-10 text-gray-800 dark:bg-gray-800 dark:text-gray-100">
        <div class="container grid grid-cols-12 mx-auto gap-y-6 gap-x-8">
            <Skeleton prefer="card" h="100%" w="100%" br="5px" class="h-full w-full col-span-12 lg:col-span-8" />
            <Skeleton prefer="card" h="100%" w="100%" br="5px" class="h-full w-full col-span-12 lg:col-span-4"/>
        </div>
    </section>

    <div v-else-if="loadingError" class="grid px-4 h-[50vh] place-content-center">
        <h1 class="tracking-widest text-gray-500 uppercase">404 | Cannot fetch: Are you connected to the internet?</h1>
    </div>

    <section v-else class="px-5 py-10 text-gray-800 dark:bg-gray-800 dark:text-gray-100">
        <div class="container grid grid-cols-12 mx-auto gap-y-6  gap-x-8">
            ...
            <div class="relative flex col-span-12 dark:bg-gray-500 md:col-span-12 lg:col-span-9 min-h-96">
                <article class="py-18 mx-auto space-y-16 text-gray-800 dark:bg-gray-800 dark:text-gray-50">
                    <div class="w-full mx-auto space-y-4">
                        ...
                        <h1 class="text-5xl font-bold">{{ articleData.title }}
                        </h1>
                        ...
                        <img :src="`${articleData.img}`" class="block w-[80%] mx-auto aspect-square max-h-60" :alt="`Project image: ${articleData.title}`">
                    </div>
                    ...
                    <div class="container mx-auto px-4 dark:text-gray-100">
                        <div class="flex flex-wrap items-center mt-16">
                            ...
                            <div
                                class="w-full self-start md:w-4/12 px-4 mx-auto relative flex flex-col min-w-0 break-words mb-2 shadow-lg rounded-lg bg-pink-500">
            ...
            <div class="py-2 col-span-12 md:col-span-10 xl:col-span-3 lg:col-span-3 lg:block">
                ...
                <div class="flex flex-col divide-y divide-gray-700">
                    ...
                    <div v-for="recentArticle in recentArticles" class="flex px-1 py-4">
                        <img :alt="recentArticle.title" class="flex-shrink-0 object-cover w-20 h-20 mr-4 dark:bg-gray-500"
                            :src="recentArticle.img">
